        ...
        if (body.tracked) {
            ...
            if ((Date.now() - timeLastPushed) > 5000) {
                ...
                html2canvas(document.querySelector("#test")).then(function (canvas2) {
                    ...
                    download('skeleton_' + Date.now(), JSON.stringify(frames))

                    // toDataURL defaults to png, so we need to request a jpeg, then convert for file download.
                    // a.href = canvas2.toDataURL("image/jpeg").replace("image/jpeg", "image/octet-stream");
                    // a.download = 'skeleton_'+ Date.now() +'.jpg';
                    // a.click();
                    count++;
                })
